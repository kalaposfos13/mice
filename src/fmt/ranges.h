...
class is_tuple_formattable_<T, C, true> {
    template <size_t... Is>
    static auto all_true(index_sequence<Is...>, integer_sequence<bool, (Is >= 0)...>)
        -> std::true_type;
    ...
    static auto check(index_sequence<Is...>) -> decltype(all_true(
        index_sequence<Is...>{},
        integer_sequence<
            bool, (is_formattable<typename std::tuple_element<Is, T>::type, C>::value)...>{}));
...
FMT_CONSTEXPR void for_each(Tuple&& t, F&& f) {
    for_each(tuple_index_sequence<remove_cvref_t<Tuple>>(), std::forward<Tuple>(t),
             std::forward<F>(f));
}
...
void for_each2(Tuple1&& t1, Tuple2&& t2, F&& f) {
    for_each2(tuple_index_sequence<remove_cvref_t<Tuple1>>(), std::forward<Tuple1>(t1),
              std::forward<Tuple2>(t2), std::forward<F>(f));
}
...
struct range_format_kind_
    : std::integral_constant<range_format, std::is_same<uncvref_type<T>, T>::value
...
struct formatter<
    R, Char,
    enable_if_t<conjunction<bool_constant<range_format_kind<R, Char>::value == range_format::map>,
                            detail::is_formattable_delayed<R, Char>>::value>> {
private:
...
private:
    using range_type = detail::maybe_const_range<R>;
    using string_type =
        conditional_t<std::is_constructible<detail::std_string_view<Char>,
...
public:
    ...
    auto format(range_type& range, FormatContext& ctx) const -> decltype(ctx.out()) {
        ...
        if (detail::const_check(range_format_kind<R, Char>::value == range_format::debug_string))
            *out++ = '"';
        out = underlying_.format(string_type{detail::range_begin(range), detail::range_end(range)},
                                 ctx);
        if (detail::const_check(range_format_kind<R, Char>::value == range_format::debug_string))
            *out++ = '"';
        return out;
...
private:
    ...
    using view = conditional_t<std::is_copy_constructible<It>::value,
                               const join_view<It, Sentinel, Char>, join_view<It, Sentinel, Char>>;
...
struct formatter<tuple_join_view<Char, Tuple>, Char, enable_if_t<is_tuple_like<Tuple>::value>> {
    ...
    template <typename FormatContext>
    auto format(const tuple_join_view<Char, Tuple>& value, FormatContext& ctx) const ->
...
private:
    ...
    FMT_CONSTEXPR auto do_parse(parse_context<Char>& ctx, std::integral_constant<size_t, 0>)
        -> const Char* {
        return ctx.begin();
    }

    template <size_t N>
    FMT_CONSTEXPR auto do_parse(parse_context<Char>& ctx, std::integral_constant<size_t, N>)
        -> const Char* {
        auto end = ctx.begin();
