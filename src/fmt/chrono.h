...
void write_codecvt(codecvt_result<CodeUnit>& out, string_view in, const std::locale& loc) {
    ...
    auto result =
        f.in(mb, in.begin(), in.end(), from_next, std::begin(out.buf), std::end(out.buf), out.end);
...
auto duration_cast(std::chrono::duration<FromRep, FromPeriod> from) -> To {
    ...
    using common_rep =
        typename std::common_type<FromRep, typename To::rep, decltype(factor::num)>::type;
...
auto to_time_t(sys_time<Duration> time_point) -> std::time_t {
    ...
    // overflow.
    return detail::duration_cast<std::chrono::duration<std::time_t>>(time_point.time_since_epoch())
        .count();
}
...
FMT_CONSTEXPR auto parse_chrono_format(const Char* begin, const Char* end, Handler&& handler)
    -> const Char* {
    if (begin == end || *begin == '}')
        return begin;
    if (*begin != '%')
...
public:
    constexpr explicit tm_format_checker(bool has_timezone) : has_timezone_(has_timezone) {}
...
public:
    tm_writer(const std::locale& loc, OutputIt out, const std::tm& tm,
        ...
        : loc_(loc), is_classic_(loc_ == get_classic_locale()), out_(out), subsecs_(subsecs),
          tm_(tm) {}
    ...
    void on_second(numeric_system ns, pad_type pad) {
        ...
        if (ns == numeric_system::standard) {
            if (std::is_floating_point<rep>::value) {
                ...
                write(second(), 2, pad);
                write_fractional_seconds<Char>(out, std::chrono::duration<rep, Period>(val),
                                               precision);
            }
            return;
        }
...
protected:
    ...
    auto do_format(const std::tm& tm, FormatContext& ctx, const Duration* subsecs) const
        -> decltype(ctx.out()) {
        auto specs = specs_;
        auto buf = basic_memory_buffer<Char>();
        auto out = basic_appender<Char>(buf);
