...
public:
    ...
    auto format(const std::filesystem::path& p, FormatContext& ctx) const {
        ...
        return detail::write(ctx.out(), basic_string_view<Char>(quoted.data(), quoted.size()),
                             specs);
    }
};
...
struct formatter<std::expected<T, E>, Char,
                 std::enable_if_t<(std::is_void<T>::value || is_formattable<T, Char>::value) &&
...
struct formatter<Variant, Char,
    ...
    auto format(const Variant& value, FormatContext& ctx) const -> decltype(ctx.out()) {
        ...
        FMT_TRY {
            std::visit(
                [&](const auto& v) { out = detail::write_escaped_alternative<Char>(out, v); },
                value);
        }
...
private:
    ...
    FMT_CONSTEXPR auto do_format(const std::complex<T>& c,
                                 detail::dynamic_format_specs<Char>& specs, FormatContext& ctx,
